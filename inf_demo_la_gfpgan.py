...
from utils.data_avhubert import collater_audio, emb_roi2im, emb_roi2im2
...
def restored_pred(ffs,bbxs=None):

    gfpgan_pth = r'/workdir/GFPGAN'
    sys.path.append(gfpgan_pth)
    from gfpgan.utils import GFPGANer

    restorer = GFPGANer(model_path='/workdir/GFPGAN/experiments/pretrained_models/GFPGANv1.3.pth', 
    ...
    for ff in ffs:
        cropped_faces, restored_faces, restored_img = restorer.enhance(
            ff.cpu().numpy().clip(0,255).astype(np.uint8), has_aligned=False, only_center_face=True, paste_back=True)
...
def synt_demo(face_detector, device, model, args):
    ...
    processed_img = emb_roi2im2([idAudio], imgs, bbxs, prediction, device)
    torch.cuda.empty_cache()
    # restored_img = restored_pred(processed_img[0],bbxs=None)
    

    # processed_img = inpatient(processed_img[0], imgs[0], bbxs[0])


    end_time = time.time()
    print('inpatient time:',end_time-start_time)
    start_time = time.time()
    ...
    for j, im in enumerate(tqdm(processed_img[0])):
        ...
        out.write(im)
    # for j, im in enumerate(restored_img):
